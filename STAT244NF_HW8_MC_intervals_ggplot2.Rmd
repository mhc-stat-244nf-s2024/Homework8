...
SEIR_simulation <- function(N, S0, E0, I0, RN, pD, D, TotalTime, stochastic=FALSE){
  ...
  if (stochastic==TRUE){
    ...
    for (t in 2:(TotalTime+1)){
      ...
      SEIR_df$E[t] <- SEIR_df$E[t-1]+SEIR_df$lambda_t[t]*SEIR_df$S[t-1]-risk_pi*SEIR_df$I[t-1]
      SEIR_df$I[t] <- SEIR_df$I[t-1]+risk_pi*SEIR_df$I[t-1]-risk_rho*SEIR_df$R[t-1]
      SEIR_df$R[t] <- SEIR_df$R[t-1]+risk_rho*SEIR_df$R[t-1]
    }
  }
...
**LD 2. Do the same thing as in LD 1, except this time, conduct 50 simulations, rather than 10, save the object as `SEIR_sim_100`, and print the *last* 10 rows of your new data frame using the `tail` function. Check out the help documentation if you are unfamiliar with this function.**
